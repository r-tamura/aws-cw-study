...
**AWS 公式ドキュメント**
- [Generative AI observability](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/GenAI-observability.html) — Model Invocations / Bedrock AgentCore タブと前提機能（Transaction Search・Model invocation logging）の正本
- [View observability data in CloudWatch (AgentCore)](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/view-observability-data-cloudwatch.html) — `bedrock-agentcore` 名前空間のメトリクスとセッション/トレース/スパンの 3 階層
- [AgentCore generated runtime observability data](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability-runtime-metrics.html) — Runtime / Memory / Gateway / Tools / Identity の primitives 単位メトリクスの一覧

**AWS ブログ / アナウンス**
- [Launching Amazon CloudWatch generative AI observability (Preview)](https://aws.amazon.com/blogs/mt/launching-amazon-cloudwatch-generative-ai-observability-preview/) — Strands Agents / LangGraph / CrewAI など OSS フレームワーク連携と ADOT 自動計装の発表（2025/07）
- [Build trustworthy AI agents with Amazon Bedrock AgentCore Observability](https://aws.amazon.com/blogs/machine-learning/build-trustworthy-ai-agents-with-amazon-bedrock-agentcore-observability/) — AgentCore Observability の OpenTelemetry ベースの設計思想（2025/07）
- [Enhance your AIOps: Introducing Amazon CloudWatch and Application Signals MCP servers](https://aws.amazon.com/blogs/mt/enhance-your-aiops-introducing-amazon-cloudwatch-and-application-signals-mcp-servers/) — IDE / Slack / GitHub の `@awsapm` から自然言語で観測データを叩く経路の発表（2025/11）

**OSS / 標準仕様**
- [OpenTelemetry Semantic Conventions for Generative AI](https://opentelemetry.io/docs/specs/semconv/gen-ai/) — `gen_ai.system` / `gen_ai.usage.*` / `gen_ai.response.finish_reasons` 等の属性定義の上流仕様
