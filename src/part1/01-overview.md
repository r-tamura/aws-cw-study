...
flowchart TD
    subgraph Sources["観測対象"]
        ...
        App["アプリケーション<br/>(EC2 / ECS / EKS / Lambda)"]
        OnPrem["オンプレ・他クラウド"]
        Browser["ブラウザ / モバイル端末"]
    end
    ...
    subgraph App["アプリケーション・オブザーバビリティ"]
...
トレンドとしては (1) **AI 統合**（Investigations / Q チャット）、(2) **OTel 一級化**（OTLP エンドポイント、PromQL）、(3) **クロスアカウント運用の自動化**（Centralization / Telemetry config）の 3 軸で機能拡張が進んでいます。
